import { SoftwareApplicationJsonLdProps } from "next-seo";

export const softwareApplications: SoftwareApplicationJsonLdProps[] = [
  {
    type: "WebApplication",
    name: "Mukesh Murugaiyan Portfolio",
    description:
      "Official portfolio web application of Mukesh Murugaiyan (TheMukesh), a Full Stack Developer showcasing web, mobile and desktop applications.",
    url: "https://themukesh.com",
    image: "https://themukesh.com/mukesh-mg2.png",
    applicationCategory: "WebApplication",
    applicationSubCategory: "DeveloperPortfolio",
    applicationSuite: "Personal Portfolio",
    operatingSystem: "All",
    availableOnDevice: "Desktop, Mobile",
    permissions: "No special permissions required",
    softwareVersion: "1.0.0",
    releaseNotes: "Continuous updates with new projects and features",
    featureList: [
      "Developer portfolio",
      "Project showcase",
      "Resume download",
      "Contact form",
      "Freelance services",
      "Mobile & web app demos",
    ],
    countriesSupported: "Worldwide",
    author: {
      "@type": "Person",
      name: "Mukesh Murugaiyan",
    },
    publisher: {
      "@type": "Organization",
      name: "Mukesh Murugaiyan",
      url: "https://themukesh.com",
    },
    datePublished: "2020-01-01",
    dateModified: "2025-12-06",
    contentRating: "General",
  },
  {
    type: "MobileApplication",
    name: "Mukesh Murugaiyan",
    description:
      "High-performance Android applications built using React Native, Kotlin, and Node.js backend | by Full Stack Developer Mukesh Murugaiyan.",
    url: "https://themukesh.com",
    applicationCategory: "MobileApplication",
    operatingSystem: "Android",
    availableOnDevice: "Mobile",
    permissions: "No special permissions required",
    softwareVersion: "1.0.0",
    releaseNotes: "Continuous updates with new projects and features",
    featureList: [
      "Android app development",
      "High-performance mobile apps",
      "React Native apps",
    ],
    countriesSupported: "Worldwide",
    author: {
      "@type": "Person",
      name: "Mukesh Murugaiyan",
    },
    publisher: {
      "@type": "Organization",
      name: "Mukesh Murugaiyan",
      url: "https://themukesh.com",
    },
    datePublished: "2020-01-01",
    dateModified: "2025-12-06",
    contentRating: "General",
  },
  {
    type: "MobileApplication",
    name: "Mukesh Murugaiyan",
    description:
      "Modern and high-quality iOS applications developed using React Native and Node.js backend | by Full Stack Developer Mukesh Murugaiyan.",
    url: "https://themukesh.com",
    applicationCategory: "MobileApplication",
    operatingSystem: "iOS",
    availableOnDevice: "Mobile",
    permissions: "No special permissions required",
    softwareVersion: "1.0.0",
    releaseNotes: "Continuous updates with new projects and features",
    featureList: [
      "iOS app development",
      "High-performance mobile apps",
      "React Native apps",
    ],
    countriesSupported: "Worldwide",
    author: {
      "@type": "Person",
      name: "Mukesh Murugaiyan",
    },
    publisher: {
      "@type": "Organization",
      name: "Mukesh Murugaiyan",
      url: "https://themukesh.com",
    },
    datePublished: "2020-01-01",
    dateModified: "2025-12-06",
    contentRating: "General",
  },
  {
    type: "DesktopEnhancementApplication",
    name: "Mukesh Murugaiyan",
    description:
      "High-quality desktop applications built using Electron.js and Node.js backend | by Full Stack Developer Mukesh Murugaiyan.",
    url: "https://themukesh.com",
    applicationCategory: "DesktopEnhancementApplication",
    operatingSystem: "Windows, macOS, Linux",
    availableOnDevice: "Desktop",
    permissions: "No special permissions required",
    softwareVersion: "1.0.0",
    releaseNotes: "Continuous updates with new projects and features",
    featureList: [
      "Desktop app development",
      "Electron.js apps",
      "Cross-platform desktop applications",
    ],
    countriesSupported: "Worldwide",
    author: {
      "@type": "Person",
      name: "Mukesh Murugaiyan",
    },
    publisher: {
      "@type": "Organization",
      name: "Mukesh Murugaiyan",
      url: "https://themukesh.com",
    },
    datePublished: "2020-01-01",
    dateModified: "2025-12-06",
    contentRating: "General",
  },
  {
    type: "WebApplication",
    name: "AI Image Background Remover",
    applicationCategory: "DesignApplication",
    operatingSystem: "All",
    description: "Instantly and privately remove image backgrounds for free using client-side AI by Mukesh Murugaiyan.",
    url: "https://themukesh.com/tools/image-bg-remover",
    author: { "@type": "Person", name: "Mukesh Murugaiyan" },
  },
  {
    type: "WebApplication",
    name: "Online API Tester",
    applicationCategory: "DeveloperTool",
    operatingSystem: "All",
    description: "Multi-platform HTTP client for testing APIs and inspecting JSON responses in real-time.",
    url: "https://themukesh.com/tools/api-tester",
    author: { "@type": "Person", name: "Mukesh Murugaiyan" },
  },
  {
    type: "WebApplication",
    name: "App Icon Generator",
    applicationCategory: "DeveloperTool",
    operatingSystem: "All",
    description: "Generate Xcode-ready iOS assets and Android adaptive icons instantly from your browser.",
    url: "https://themukesh.com/tools/app-icon-generator",
    author: { "@type": "Person", name: "Mukesh Murugaiyan" },
  },
  {
    type: "WebApplication",
    name: "Social Media Downloader",
    applicationCategory: "MultimediaApplication",
    operatingSystem: "All",
    description: "High-quality downloader for Instagram Reels and Pinterest videos.",
    url: "https://themukesh.com/tools/instagram-downloader",
    author: { "@type": "Person", name: "Mukesh Murugaiyan" },
  },
];
